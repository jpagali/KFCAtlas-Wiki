...
:::caution Helium does not own the menu
Helium renders the menu it receives from the Menu Service. Prices, item descriptions, availability, and modifiers are all managed in **Byte Menu** (the menu authoring tool) and assigned via the **Portal**. If something is wrong with the menu, the fix is not in Helium.
:::
...
|---|---|---|---|---|---|---|---|
| **View Localised Menu (PLP)** | Customers need to see what's available | Displays the product listing page with categories and items for the selected store/channel | Helium calls Menu Service with store context → gets category/item list → renders | Menu Service, Context service | Localisation must be completed first | Pricing is market-specific; Helium does NOT own pricing; PLP p95 ≤200ms | Menu impression rate, scroll depth |
| **View Item Detail (PDP)** | Customer needs full item info before ordering | Shows item name, description, image, price, modifiers, and nutritional info | Helium calls Menu Service with item ID + context → gets full item detail | Menu Service | PLP viewed; item in menu | Options filtered by context (e.g., delivery-only items hidden for collection) | PDP view rate, modifier selection rate |
...
| **Nutrition & Allergens Guide** | Customers need dietary information | Shows full nutritional breakdown and allergen information for menu items | Nutrition data fetched from dedicated Nutrition index/detail endpoints | Nutrition service (via Menu) | Menu must include nutrition data (configured in Byte Menu) | Data must be authored and maintained in Byte Menu — not a Helium responsibility | Guide engagement rate |
| **FAQ** | Customers have questions that aren't orders | Renders a searchable FAQ page | CMS API returns FAQ entries for the market locale; search is client-side | CMS service | FAQ content authored in CMS (Portal) | Content quality depends on what markets publish in CMS | FAQ page visits, search queries |
...
- **Source Summary:**
  ...
  - Pricing is market-specific and owned by Commerce Backend — Helium renders only
...
- Managing menu items and modifiers in the Portal → [Admin Portal Guide: Products](/docs/admin-portal-guide/products/) and [Menus](/docs/admin-portal-guide/menus/)
:::
