...
| Term | Definition |
|---|---|
| **Byte Helium** | The customer-facing web/app front-end for KFC ordering |
| **Commerce Backend** | The engine that processes orders, pricing, payments, and POS injection; if a market is not on Byte POS, Byte Connect is required as the bridge to the market POS |
| **Byte Portal** | The admin interface for market and ops teams to configure the platform |
| **Byte Menu** | The menu authoring tool — external to Atlas; Byte Portal assigns published menus |
| **Byte Connect** | The integration bridge used when Byte Commerce needs to reach a non-Byte POS environment; it also supports aggregator order flows into Commerce Backend |
...
| **POS** | Point of Sale — the in-store order management system; Byte Commerce talks directly to Byte POS, and uses Byte Connect when a market runs a non-Byte POS |
...
| **Aggregator** | A third-party delivery platform (e.g., Uber Eats, DoorDash) whose orders are ingested via Byte Connect into Commerce Backend |
